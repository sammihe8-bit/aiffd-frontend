...
const CHEEKBONE_PROMINENCE_TO_SCHEMA: Record<string, string> = { low: 'low', medium: 'moderate', high: 'prominent' }
const CHEEKBONE_SHAPE_TO_SCHEMA: Record<string, string> = { round: 'rounded', balanced: 'balanced', angular: 'angular' }
...
export default function StyleTestPage() {
  ...
  const finishAll = () => {
    ...
    // 2026-09-16 新增：双写进 Human Profile DB，跟上面三处 localStorage 存档并行，不影响现有流程。
    // 面部字段从 imageComboSelections 读取（原始英文选项 id），不是从 faceAnswers 读取
    // （faceAnswers 存的是拼好的中文展示标签，不能直接喂给后端的英文枚举字段）。
    // jawline（下颌线条）和 faceLine（面部整体线条）这两项是数据字典里已知的题库缺口，
    // 现有题目答不出干净的值，这里先不发，等题库补齐后再接上，避免塞进不准确的数据。
    const lip = imageComboSelections.lip
    const cheek = imageComboSelections.cheek
    const cheekbone = imageComboSelections.cheekbone
    const chin = imageComboSelections.chin
    const eyesSel = imageComboSelections.eyes
    const noseSel = imageComboSelections.nose

    const humanProfilePatch: Record<string, unknown> = {}
    if (lip?.width) humanProfilePatch.mouthWidth = lip.width
    if (lip?.fullness) humanProfilePatch.mouthFullness = lip.fullness
    if (cheek?.contour) humanProfilePatch.cheekContour = cheek.contour
    if (cheek?.fullness) humanProfilePatch.cheekFullness = CHEEK_FULLNESS_TO_SCHEMA[cheek.fullness] ?? cheek.fullness
    if (cheekbone?.prominence) humanProfilePatch.cheekboneProminence = CHEEKBONE_PROMINENCE_TO_SCHEMA[cheekbone.prominence] ?? cheekbone.prominence
    if (cheekbone?.contour) humanProfilePatch.cheekboneShape = CHEEKBONE_SHAPE_TO_SCHEMA[cheekbone.contour] ?? cheekbone.contour
    if (chin?.length) humanProfilePatch.chinLength = chin.length
    if (chin?.tip) humanProfilePatch.chinShape = chin.tip
    if (eyesSel?.size) humanProfilePatch.eyeSize = eyesSel.size
    if (eyesSel?.shape) humanProfilePatch.eyeShape = eyesSel.shape
    if (eyesSel?.spacing) humanProfilePatch.eyeSpacing = eyesSel.spacing
