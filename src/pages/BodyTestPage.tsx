...
function calcFatCode(whr: number, bustWaistDiff: number, visual: string, boneCode: string): string {
  ...
  else if (bustWaistDiff <= 15) { noneScore += 30 } else { oScore += 15; noneScore += 15 }
...
export default function BodyTestPage() {
  ...
  const checkConflict = (b: string, w: string, h: string) => {
    ...
    const whr = wN / hN
    const bwd = bN - wN
    if (whr > 0.9 && bwd > 25) setConflict('腰臀比偏高但胸腰差较大，数据存在轻微冲突，建议重新测量腰围确认。')
    else setConflict('')
  }
  ...
  const computeResult = () => {
    ...
    const shoulderN = parseFloat(shoulder) || 38
    const hipBoneN = parseFloat(hipBone) || 36
  ...
  const startAiAnalysis = () => {
    ...
    setTimeout(() => setAiStatus('done'), 3000)
  }
  ...
  const btnPrimary = {
    width: '100%', padding: '16px', background: C.h1, color: '#fff',
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '64px 24px 96px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {[
                ...
                <button key={m.id} onClick={() => { setMethod(m.id as 'manual' | 'ai'); next() }} style={{
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '4px', color: C.gold, marginBottom: '12px' }}>Step 01 · 体质底层</p>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '28px', fontWeight: 400, color: C.h1, marginBottom: '8px' }}>你的体质类型更接近哪种？</h2>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', color: C.muted, marginBottom: '40px' }}>选择最接近你天生体质倾向的类型（非当前体重状态）</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
              {[
                { id: 'Ectomorph', label: '外胚型', en: 'Ectomorph', keywords: ['纤细', '骨骼清晰', '代谢快'], desc: '身体线条垂直纤细，骨骼可见，脂肪极少，新陈代谢快，增重困难' },
                ...
                <button key={t.id} onClick={() => { setSheldon(t.id); next() }} style={{
                  ...
                  <div style={{ height: '100px', background: '#f5f2ed', marginBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'space-between' }}>
              <button onClick={prev} style={{ border: `1px solid ${C.border}`, background: '#fff', padding: '12px 24px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '12px', color: C.body }}>上一步</button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '4px', color: C.gold, marginBottom: '12px' }}>Step 02 · 骨骼轮廓</p>
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '28px', fontWeight: 400, color: C.h1, marginBottom: '8px' }}>你的骨骼轮廓更接近哪种？</h2>
        ...
        {step === 3 && (
          <div>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '4px', color: C.gold, marginBottom: '12px' }}>Step 03 · 脂肪分布</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '32px' }}>
              {[
                ...
                <button key={v.id} onClick={() => { setVisual(v.id); next() }} style={{
        ...
        {/* ── STEP 4: 手动三围 ── */}
        {step === 4 && method === 'manual' && (
          <div>
            <ProgressBar current={4} total={6} label="BODY TEST · STEP 04" />
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '4px', color: C.gold, marginBottom: '12px' }}>Step 04 · 数据输入</p>
            ...
            <div style={{ background: '#f7f4ef', padding: '20px', marginBottom: '28px', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
              {[
                ...
                <div key={m.label}>
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '1px', color: C.gold, marginBottom: '4px' }}>{m.label}</p>
        ...
        {step === 4 && method === 'ai' && (
          <div>
            <ProgressBar current={4} total={6} label="BODY TEST · STEP 04" />
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '4px', color: C.gold, marginBottom: '12px' }}>Step 04 · AI 识别</p>
            ...
            {aiStatus === 'idle' && (
              <div>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={prev} style={{ border: `1px solid ${C.border}`, background: '#fff', padding: '16px 24px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '12px', color: C.body }}>上一步</button>
                  <button onClick={startAiAnalysis} style={{ flex: 1, ...btnPrimary }}>开始 AI 分析</button>
                </div>
              </div>
            )}
            {aiStatus === 'analyzing' && (
            ...
            {aiStatus === 'done' && (
              <div>
                <div style={{ border: `1px solid ${C.gold}`, padding: '24px', marginBottom: '20px' }}>
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', letterSpacing: '3px', color: C.gold, marginBottom: '16px' }}>AI 识别结果（模拟）</p>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
                    {[{ label: '骨骼代码', value: 'H 型', conf: '82%' }, { label: '脂肪代码', value: 'O 型', conf: '76%' }, { label: '复合代码', value: 'H-O', conf: '' }].map(r => (
                      <div key={r.label}>
                        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', letterSpacing: '2px', color: C.muted, marginBottom: '4px' }}>{r.label}</p>
                        <p style={{ fontFamily: 'Georgia, serif', fontSize: '22px', color: C.gold }}>{r.value}</p>
                        {r.conf && <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', color: C.muted }}>置信度 {r.conf}</p>}
                      </div>
                    ))}
                  </div>
                </div>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: C.muted, marginBottom: '16px', lineHeight: '1.7' }}>如识别结果与你的感知不符，可切换手动输入进行验证。</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <button onClick={() => setMethod('manual')} style={{ border: `1px solid ${C.border}`, background: '#fff', padding: '14px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '13px', color: C.body }}>切换手动验证</button>
                  <button onClick={() => { setBoneShape('H'); setVisual('O'); next() }} style={{ border: 'none', background: C.h1, padding: '14px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '13px', color: '#fff' }}>确认结果，继续</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button onClick={computeResult} disabled={!q1 || !q2 || !q3}
                style={{ flex: 1, ...(!q1 || !q2 || !q3 ? btnDisabled : btnPrimary) }}>
                生成我的体型档案
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ border: `1px solid ${C.gold}`, marginBottom: '32px' }}>
              ...
              <div style={{ padding: '24px 28px' }}>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', letterSpacing: '3px', color: C.gold, marginBottom: '20px' }}>穿搭策略（完整版）</p>
                {[
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px' }}>
              <button onClick={() => { setStep(0); setResult(null) }} style={{ border: `1px solid ${C.border}`, background: '#fff', padding: '14px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: '12px', color: C.body }}>重新测试</button>
