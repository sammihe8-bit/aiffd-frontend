...
const FEATURED = [
  {
    ...
    author: 'Sammi', avatar: '/author-sammi.jpg', date: '2026.05.01', readTime: '6 分钟',
...
const SECONDARY = [
  {
    ...
    author: 'Lin Wei', avatar: '/author-linwei.jpg', date: '2026.04.20',
...
const ARTICLES = [
  { tag: '穿衣哲学', title: '断舍离之后，我的衣橱反而变得更贵了', excerpt: '减少才能看见真正想要的。一次清空，让我终于明白自己的风格。', author: 'Sammi', avatar: '/author-sammi.jpg', date: '2026.04.08', readTime: '4 分钟', img: '/article-1.jpg', link: 'https://www.ohsammi.com' },
  { tag: '色彩', title: '秋冬色彩指南：找到你的暖棕调', excerpt: '不是每个人都适合黑色。暖棕调可能才是你的本命配色。', author: 'Lin Wei', avatar: '/author-linwei.jpg', date: '2026.03.25', readTime: '5 分钟', img: '/article-2.jpg', link: 'https://www.ohsammi.com' },
  { tag: '购物决策', title: '买衣服前，我开始问自己这三个问题', excerpt: '这三个问题让我的冲动消费减少了80%，也让每件新衣都物尽其用。', author: 'Mia Zhang', avatar: '/author-mia.jpg', date: '2026.03.10', readTime: '4 分钟', img: '/article-3.jpg', link: 'https://www.ohsammi.com' },
  { tag: '40+', title: '职场转型期，我如何重建自己的穿衣语言', excerpt: '身份变了，衣橱也要跟着变。不是招贴，而是重新认识自己。', author: 'Sammi', avatar: '/author-sammi.jpg', date: '2026.03.01', readTime: '6 分钟', img: '/article-4.jpg', link: 'https://www.ohsammi.com' },
  { tag: '体型', title: 'H型骨架的人，为什么总觉得穿什么都不好看', excerpt: '问题不在身材，在于你还没找到属于 H 型骨架的建筑逻辑。', author: 'AIFFD', avatar: '/column-aiffd.jpg', date: '2026.02.20', readTime: '3 分钟', img: '/article-5.jpg', link: 'https://www.ohsammi.com' },
  { tag: '极简', title: '一个衣橱里到底需要多少件衣服？', excerpt: '我的答案是：20件。而且这 20 件里有10件是基础款。', author: 'Mia Zhang', avatar: '/author-mia.jpg', date: '2026.02.10', readTime: '4 分钟', img: '/article-6.jpg', link: 'https://www.ohsammi.com' },
]

function ImgBox({ src, alt, height }: { src: string; alt: string; height: string }) {
  return (
    <div style={{ background: '#EAE4DC', overflow: 'hidden', position: 'relative', height }}>
...
function Tag({ label }: { label: string }) {
  return <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', padding: '3px 10px', border: '0.5px solid #e8e8e4', color: '#999999', borderRadius: '20px', display: 'inline-block' }}>{label}</span>
}
    ...
    <div style={{ background: '#fafaf8', minHeight: '100vh' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '64px 64px 96px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: '#e8e8e4', marginBottom: '64px' }}>
          {FEATURED.map(f => (
            <a key={f.title} href={f.link} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'block', background: '#fafaf8' }}>
              <ImgBox src={f.img} alt={f.title} height="320px" />
              <div style={{ padding: '28px' }}>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                  <Tag label={f.tag} />{f.tag2 && <Tag label={f.tag2} />}
                </div>
                <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '22px', fontWeight: 400, color: '#111111', marginBottom: '12px', lineHeight: 1.4 }}>{f.title}</h2>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '15px', color: '#444444', lineHeight: '1.85', marginBottom: '20px' }}>{f.excerpt}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <Avatar src={f.avatar} />
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '13px', color: '#666666' }}>{f.author}</span>
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: '#999999', marginLeft: 'auto' }}>{f.date} · {f.readTime}</span>
                </div>
              </div>
            </a>
          ))}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: '#e8e8e4' }}>
            {SECONDARY.map(s => (
              <a key={s.title} href={s.link} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'grid', gridTemplateColumns: '140px 1fr', background: '#fafaf8', flex: 1 }}>
                <ImgBox src={s.img} alt={s.title} height="100%" />
                <div style={{ padding: '20px', minHeight: '160px' }}>
                  <div style={{ marginBottom: '12px' }}><Tag label={s.tag} /></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1px', background: '#e8e8e4', marginBottom: '64px' }}>
          {AUTHORS.map(a => (
            <div key={a.name} style={{ background: '#fafaf8', padding: '28px 24px' }}>
              ...
              {a.siteUrl ? (
                <a href={a.siteUrl} target="_blank" rel="noopener noreferrer" style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: '#B8973A', letterSpacing: '1px', display: 'block', marginBottom: '12px', textDecoration: 'none' }}>{a.site} ↗</a>
              ) : (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1px', background: '#e8e8e4' }}>
          {ARTICLES.map(a => (
            <a key={a.title} href={a.link} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', display: 'block', background: '#fafaf8' }}>
              <ImgBox src={a.img} alt={a.title} height="180px" />
